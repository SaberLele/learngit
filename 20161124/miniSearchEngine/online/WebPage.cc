...
namespace zx
{
	...
	//	std::cout << _docUrl << std::endl;
		...
		pos1 = doc.find("<content>",0);
		pos2 = doc.find("</content>",0);
		_docContent = doc.substr(pos1+9,pos2-pos1-9);
...
//		set<string> stopWords = config.getStopWordList();
	...
	string WebPage::summary(const vector<string> & queryWords)
	{
		...
		for(auto & elem2 : summaryVec)
		{
			_docSummary.append(elem2).append("\n");
		}
		return _docSummary;
	}
